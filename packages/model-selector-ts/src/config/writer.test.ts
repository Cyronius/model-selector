...
describe('Config Writer', () => {
  ...
  describe('validateModelConfig', () => {
    ...
    it('accepts a match-only entry with just attributes', () => {
      // Client fields (provider, model_id) are optional under the match-only contract.
      const result = validateModelConfig({ attributes: { speed: 7, cost: 2 } });
      expect(result.valid).toBe(true);
    });

    it('rejects malformed attributes', () => {
      const result = validateModelConfig({ provider: 'openai', attributes: 123 });
  ...
  describe('validateConfig', () => {
    ...
    it('rejects invalid models', () => {
      const result = validateConfig({
        aliases: {},
        models: { bad: { attributes: 123 } }, // attributes must be an object
      });
      expect(result.valid).toBe(false);
    });
  });
  ...
  describe('writeConfig', () => {
    ...
    it('returns error for invalid config', async () => {
      const result = await writeConfig(
        { aliases: {}, models: { bad: { attributes: 123 } } } as unknown as Config,
        { configPath }
      );
  ...
  describe('addModel', () => {
    ...
    it('validates model config', async () => {
      const result = await addModel(
        'bad',
        { provider: 'openai', attributes: 123 } as unknown as ModelConfig,
        { configPath }
      );
      expect(result.success).toBe(false);
